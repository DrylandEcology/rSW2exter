-- T-SQL to return one row per horizon for all horizons of the queried component of queried mukeys (inputed to '%s')

-- match specific mukey - compname combinations, see https://stackoverflow.com/questions/6672665/doing-a-where-in-on-multiple-columns-in-postgresql
...
INNER JOIN (VALUES %s) AS t (mm, cn) ON t.mm = mapunit.mukey AND t.cn = component.compname

WHERE component.compkind NOT IN ('Miscellaneous area', 'NOTCOM')
